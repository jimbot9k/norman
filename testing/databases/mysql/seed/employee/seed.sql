-- Create database (schema equivalent in MySQL)
CREATE DATABASE IF NOT EXISTS employee;
USE employee;

-- Create user with access
CREATE USER IF NOT EXISTS 'Jimbobby'@'%' IDENTIFIED BY 'Jimbobby';
GRANT SELECT, INSERT, UPDATE, DELETE ON employee.* TO 'Jimbobby'@'%';
FLUSH PRIVILEGES;


CREATE TABLE employees (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    email VARCHAR(100) UNIQUE NOT NULL,
    annual_salary DECIMAL(10, 2) NOT NULL,
    CONSTRAINT employees_email_check CHECK (email LIKE '%_@__%.__%'),
    CONSTRAINT employees_annual_salary_check CHECK (annual_salary >= 0)
);
CREATE INDEX idx_employees_name ON employees (name);


CREATE TABLE departments (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    location VARCHAR(100) NOT NULL
);
CREATE INDEX idx_departments_name ON departments (name);


CREATE TABLE positions (
    id INT AUTO_INCREMENT PRIMARY KEY,
    title VARCHAR(100) NOT NULL,
    level INT NOT NULL,
    CONSTRAINT positions_level_check CHECK (level >= 1 AND level <= 10)
);
CREATE INDEX idx_positions_title ON positions (title);
CREATE INDEX idx_positions_level ON positions (level);


CREATE TABLE employee_department (
    employee_id INT NOT NULL,
    department_id INT NOT NULL,
    position INT,
    joined_date DATE,
    parent_department_id INT,
    PRIMARY KEY (employee_id, department_id),
    FOREIGN KEY (employee_id) REFERENCES employees(id),
    FOREIGN KEY (department_id) REFERENCES departments(id),
    FOREIGN KEY (position) REFERENCES positions(id)
);
CREATE INDEX idx_employee_department_position ON employee_department (position);
CREATE INDEX idx_employee_department_joined_date ON employee_department (joined_date);
CREATE INDEX idx_employee_department_parent_department_id ON employee_department (parent_department_id);


CREATE VIEW employee_overview AS
SELECT e.id AS employee_id,
       e.name AS employee_name,
       e.email AS employee_email,
       e.annual_salary AS employee_annual_salary,
       d.name AS department_name,
       p.title AS position_title,
       p.level AS position_level,
       ed.joined_date AS date_joined
FROM employees e
LEFT JOIN employee_department ed ON e.id = ed.employee_id
LEFT JOIN departments d ON ed.department_id = d.id
LEFT JOIN positions p ON ed.position = p.id;


-- MySQL doesn't have materialized views, using a regular table instead
CREATE TABLE total_salary_by_department (
    department_id INT PRIMARY KEY,
    department_name VARCHAR(100),
    total_annual_salary DECIMAL(15, 2)
);


DELIMITER //

-- Procedure for manual full refresh (can't be called from triggers due to implicit commit)
CREATE PROCEDURE refresh_total_salary_by_department()
BEGIN
    DELETE FROM total_salary_by_department;
    INSERT INTO total_salary_by_department (department_id, department_name, total_annual_salary)
    SELECT d.id AS department_id,
           d.name AS department_name,
           COALESCE(SUM(e.annual_salary), 0) AS total_annual_salary
    FROM departments d
    LEFT JOIN employee_department ed ON d.id = ed.department_id
    LEFT JOIN employees e ON ed.employee_id = e.id
    GROUP BY d.id, d.name;
END //


-- Triggers use incremental updates instead of calling procedure
CREATE TRIGGER trg_refresh_total_salary_by_department_insert
AFTER INSERT ON employee_department
FOR EACH ROW
BEGIN
    INSERT INTO total_salary_by_department (department_id, department_name, total_annual_salary)
    SELECT d.id, d.name, COALESCE(SUM(e.annual_salary), 0)
    FROM departments d
    LEFT JOIN employee_department ed ON d.id = ed.department_id
    LEFT JOIN employees e ON ed.employee_id = e.id
    WHERE d.id = NEW.department_id
    GROUP BY d.id, d.name
    ON DUPLICATE KEY UPDATE 
        total_annual_salary = VALUES(total_annual_salary);
END //

CREATE TRIGGER trg_refresh_total_salary_by_department_update
AFTER UPDATE ON employee_department
FOR EACH ROW
BEGIN
    -- Update old department
    INSERT INTO total_salary_by_department (department_id, department_name, total_annual_salary)
    SELECT d.id, d.name, COALESCE(SUM(e.annual_salary), 0)
    FROM departments d
    LEFT JOIN employee_department ed ON d.id = ed.department_id
    LEFT JOIN employees e ON ed.employee_id = e.id
    WHERE d.id = OLD.department_id
    GROUP BY d.id, d.name
    ON DUPLICATE KEY UPDATE 
        total_annual_salary = VALUES(total_annual_salary);
    -- Update new department if changed
    IF OLD.department_id != NEW.department_id THEN
        INSERT INTO total_salary_by_department (department_id, department_name, total_annual_salary)
        SELECT d.id, d.name, COALESCE(SUM(e.annual_salary), 0)
        FROM departments d
        LEFT JOIN employee_department ed ON d.id = ed.department_id
        LEFT JOIN employees e ON ed.employee_id = e.id
        WHERE d.id = NEW.department_id
        GROUP BY d.id, d.name
        ON DUPLICATE KEY UPDATE 
            total_annual_salary = VALUES(total_annual_salary);
    END IF;
END //

CREATE TRIGGER trg_refresh_total_salary_by_department_delete
AFTER DELETE ON employee_department
FOR EACH ROW
BEGIN
    INSERT INTO total_salary_by_department (department_id, department_name, total_annual_salary)
    SELECT d.id, d.name, COALESCE(SUM(e.annual_salary), 0)
    FROM departments d
    LEFT JOIN employee_department ed ON d.id = ed.department_id
    LEFT JOIN employees e ON ed.employee_id = e.id
    WHERE d.id = OLD.department_id
    GROUP BY d.id, d.name
    ON DUPLICATE KEY UPDATE 
        total_annual_salary = VALUES(total_annual_salary);
END //

DELIMITER ;


-- Grant execute on procedures to the user
GRANT EXECUTE ON PROCEDURE employee.refresh_total_salary_by_department TO 'Jimbobby'@'%';


-- Initial data population
INSERT INTO employees (name, email, annual_salary) VALUES
('Alice Johnson', 'alice.johnson@example.com', 75000.00),
('Bob Smith', 'bob.smith@example.com', 68000.00),
('Charlie Kirk', 'charlie.kirk@example.com', 72000.00),
('Diana Prince', 'diana.prince@example.com', 78000.00),
('Mike Hunt', 'mike.hunt@example.com', 82000.00);

INSERT INTO departments (name, location) VALUES
('Human Resources', 'Brisbane'),
('Engineering', 'Sydney');

INSERT INTO positions (title, level) VALUES
('Manager', 5),
('Software Engineer', 4);

INSERT INTO employee_department (employee_id, department_id, position, joined_date, parent_department_id) VALUES
(1, 1, 1, '2020-01-15', NULL),
(2, 2, 2, '2019-03-22', NULL),
(3, 2, 2, '2021-07-30', NULL),
(4, 1, 1, '2018-11-05', NULL),
(5, 2, 2, '2022-02-14', NULL);

-- Initialize the materialized view equivalent
CALL refresh_total_salary_by_department();
